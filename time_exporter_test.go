...
var (
	binary = filepath.Join(os.Getenv("GOPATH"), "bin/time_exporter")
)
...
func TestFileDescriptorLeak(t *testing.T) {
	if _, err := os.Stat(binary); err != nil {
		t.Skipf("time_exporter binary not available, try to run `make build` first: %s", err)
	}
...
func TestHandlingOfDuplicatedMetrics(t *testing.T) {
	...
	dir, err := ioutil.TempDir("", "time-exporter")
	if err != nil {
		t.Fatal(err)
	}
